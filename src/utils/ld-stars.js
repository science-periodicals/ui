...
const NON_FREE_LANG = new Set(['matlab', 'mathematica']);

export default function ldstars(doc = {}) {
  var rating = {
    ol: !!isOl(doc),
    uri: !!isUri(doc),
    of: !!isOf(doc),
  ...
  Object.defineProperty(rating, 'toString', {
    enumerable: false,
    configurable: false,
    writable: false,
    value: function() {
      var s = [];
      ['ol', 're', 'of', 'uri', 'ld'].forEach(x => {
        if (this[x]) {
          s.push(x);
        }
      });
...
function isOf(r) {
  ...
  var formats = encodings.filter(x => x.fileFormat).map(x => x.fileFormat);
  ...
  if (r.programmingLanguage && r.programmingLanguage.name) {
    ...
    return !formats.every(function(f) {
      return NON_FREE.has(f);
    });
  } else {
  ...
  for (var i = 0; i < r.length; i++) {
    if (r[i]) {
      if (
        ...
        (r[i].isBasedOn &&
          (!Array.isArray(r[i].isBasedOn) || r[i].isBasedOn.length)) ||
